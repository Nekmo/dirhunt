...
class TestCatchKeyboardInterrupt(unittest.TestCase):
    def test_keyboard_interrupt(self):
        ...
        with patch(
            "dirhunt.utils.confirm_close", side_effect=KeyboardInterrupt
        ) as mock_confirm_close:
...
class TestCatchKeyboardInterruptChoices(unittest.TestCase):
    def test_keyboard_interrupt(self):
        m = Mock(side_effect=KeyboardInterrupt)
        with patch(
            "dirhunt.utils.confirm_choices_close", side_effect=KeyboardInterrupt
        ) as mock_confirm_close:
            with self.assertRaises(KeyboardInterrupt):
