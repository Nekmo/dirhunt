...
class TestRobots(unittest.TestCase):
    def test_parse(self):
        ...
        with patch.object(Robots, "add_result") as mock_add_result:
            with patch.object(
                DirhuntRobotFileParser, "read", side_effect=read, autospec=True
            ):
...
class TestVirusTotal(unittest.TestCase):
    ...
    def test_urls(self, m):
        ...
        m.get(
            url,
            text='<html><body><div id="detected-urls">{}</div></body></html>'.format(
                "\n".join(
                    [ABUSE_DIV.format(url=detect_url) for detect_url in detect_urls]
                )
            ),
        )
...
class TestGoogle(unittest.TestCase):
    ...
    def test_search(self, m1):
        ...
        with patch(
            "dirhunt.sources.google.search", side_effect=lambda x, stop: iter(urls)
        ) as m2:
...
class TestCommonCrawl(unittest.TestCase):
    ...
    def test_get_latest_craw_index(self, m):
        ...
        self.assertEqual(
            common_crawl.get_latest_craw_index(), COMMONCRAWL_INDEXES[0]["cdx-api"]
        )
...
class TestCrtSh(unittest.TestCase):
    ...
    def test_callback(self, m, add_result_mock):
        domain = "domain.com"
        m.get(
            "{}?q={}&output=json".format(CRTSH_URL, domain),
            json=[
...
class TestCertificateSSL(unittest.TestCase):
    ...
    def test_callback(self, m):
        ...
        with patch.object(CertificateSSL, "add_result") as mock_add_result:
            m.create_default_context.return_value.wrap_socket.return_value.__enter__.return_value.getpeercert.return_value = {
    ...
    @patch(
        "dirhunt.sources.ssl.ssl.create_default_context",
...
class TestWayback(unittest.TestCase):
    ...
    def test_callback(self, m, add_result_mock):
        ...
        m.get(
            "{}?{}".format(
                WAYBACK_URL, urlencode(dict(WAYBACK_PARAMS, url="*.{}".format(domain)))
            ),
            text="\n".join(subdomains),
        )
        ...
        add_result_mock.assert_has_calls(
            [call(subdomain) for subdomain in subdomains], any_order=True
        )
