...
STATUS_LEVEL = (
    3  # 1:Planning 2:Pre-Alpha 3:Alpha 4:Beta 5:Production/Stable 6:Mature 7:Inactive
)
...
requirements_path = os.path.join(
    __dir__, "py{}-requirements.txt".format(sys.version_info.major)
)
...
if os.path.exists(scripts_path):
    ...
    scripts_dir_name = (
        scripts_dir_name[1:]
        if scripts_dir_name.startswith(os.sep)
        else scripts_dir_name
    )
    scripts = [
        os.path.join(scripts_dir_name, file) for file in os.listdir(scripts_path)
    ]
else:
    scripts = []
...
CLASSIFIERS.extend(
    ["Programming Language :: Python :: %s" % version for version in PYTHON_VERSIONS]
)
CLASSIFIERS.extend(
    [
...
setup(
    ...
    keywords=KEYWORDS,
    scripts=scripts,
    entry_points={"console_scripts": ["dirhunt = dirhunt.management:main"]},
    zip_safe=False,
)
